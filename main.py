...
def get_partial_img(data):
    core, x0, y0, source_averages, data_tiles = data
    print(core, x0, y0)
    tilized_img = np.zeros(
        (y0*output_tile_size, x0*output_tile_size, 3), np.uint8)
    print(tilized_img.shape)
    for y in range(y0, core*y0):
        for x in range(x0, core*x0):
            src_tuple = eval(source_averages[x + y * core*x0])
            closest_color = ca.get_closest_color(src_tuple, data_tiles.keys())
    ...
    print('core ', core, ' done.')
    cv2.imwrite("core_" + str(core) + ".jpg", tilized_img)
...
if __name__ == '__main__':
    ...
    for y in range(0, num_tiles_h):
        for x in range(0, num_tiles_w):
            ...
            closest_color = ca.get_closest_color(
                source_tuple, data_tiles.keys())
